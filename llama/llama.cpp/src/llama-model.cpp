...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        switch (arch) {
            ...
            case LLM_ARCH_JINA_BERT_V3:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        layer.attn_out_norm   = create_tensor(tn(LLM_TENSOR_ATTN_OUT_NORM, "weight", i), {n_embd}, TENSOR_NOT_REQUIRED);
                        layer.attn_out_norm_b = create_tensor(tn(LLM_TENSOR_ATTN_OUT_NORM, "bias", i),   {n_embd}, TENSOR_NOT_REQUIRED);
                        ...
                        if (hparams.moe_every_n_layers > 0 && i % hparams.moe_every_n_layers == 1) {
                            ...
                            if (arch == LLM_ARCH_NOMIC_BERT) {
                                // NOMIC_BERT uses gated FFN (SwiGLU)
                                layer.ffn_gate = create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd, n_ff}, 0);
                            }
                        }
            ...
            case LLM_ARCH_MODERN_BERT:
                {
                    // ModernBERT tensor loading - aligned with upstream llama.cpp
                    tok_embd = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab}, 0);
                    tok_norm = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD_NORM, "weight"), {n_embd}, 0);

                    output_norm = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd}, 0);

                    // Classifier tensors for reranker support
                    cls     = create_tensor(tn(LLM_TENSOR_CLS,     "weight"), {n_embd, n_embd}, TENSOR_NOT_REQUIRED);
                    cls_out = create_tensor(tn(LLM_TENSOR_CLS_OUT, "weight"), {n_embd, hparams.n_cls_out}, TENSOR_NOT_REQUIRED);
                    cls_out_b = create_tensor(tn(LLM_TENSOR_CLS_OUT, "bias"), {hparams.n_cls_out}, TENSOR_NOT_REQUIRED);
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        // Attention norm (pre-attention, layer 0 may not have it)
                        if (i != 0) {
                        ...
                        // FFN tensors - upstream uses combined ffn_up with GEGLU
